...
import com.seekisle.commondomain.constants.CommonConstants;
...
public class JsonUtil {
     ...
     *
     * @param str 字符串信息
     * @param keyClass key的类别
     * @param valueClass vlaue的类别
     * @return map对象
     * @param <T> value的类型
     */
    public static <T> Map<String,T> string2map(String str,Class<T> keyClass,Class<T> valueClass) {
        if (StringUtils.isEmpty(str) || keyClass == null|| valueClass == null) {
            return null;
        }
        JavaType javaType = OBJECT_MAPPER.getTypeFactory().constructMapType(LinkedHashMap.class,keyClass, valueClass);
        try {
            return OBJECT_MAPPER.readValue(str, javaType);
        } catch (IOException e) {
            log.error("Parse String to Object error : {}" + e.getMessage());

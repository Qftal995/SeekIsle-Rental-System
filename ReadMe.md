...
```bash
docker compose -p house -f deploy/dev/app/docker-compose-mid.yml up -d
```

| 服务 | 端口 | 凭据 |
|------|------|------|
| MySQL 8.4 | 3306 | `dev` / `bite@123` |
| Redis 7.0 | 6379 | `bite@123` |
| RabbitMQ 3.12 | 5672 / 15672 | `jiuyeke` / `bite@123` |
...
- **管理端**：Vue 3 + Vite + Element Plus（`HouseAdmin-feature-house`）
- **小程序端**：uni-app（`housemp-develop`）
